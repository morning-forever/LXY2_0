							...
							<c:forEach items="${cars }" var="car">
								<ul class="cl">
									...
									<c:if test="${car.driverName != '' && car.driverName != null }">
										<li class="f-l ml-50 mb-15">司机（${car.driverName}|${car.driverTel }）
											<a class="btn btn-primary radius" href="javascript:void(0);" onclick="assDri(${car.id})">换个司机</a>
										</li>
									</c:if>
									<c:if test="${car.driverName == '' || car.driverName == null }">
										<li class="f-l ml-50 mb-15"><a class="btn btn-primary radius" href="javascript:void(0);" onclick="assDri(${car.id})">分配司机</a></li>
									</c:if>
...
function assDri(carStaId){
	var param = {"carStaId":carStaId,"startTime":"${orderForm.startTime}","endTime":"${orderForm.endTime}"};
	subWebForm("admin/lxy/dri_car_sta_int_ass/carStaAssDriAddUI",param);
}
function printOrder(){
	layer.open({
        type: 1,
