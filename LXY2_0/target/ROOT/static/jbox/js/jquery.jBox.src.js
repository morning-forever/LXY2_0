...
(function ($) {
    $.jBox = function (content, options) {
        ...
        if (withTitle) {
        	if(options.showTitle){
        		...
        		boxHtml.push('<div class="jbox-title-panel" style="height:25px;display:none;">');
        	}
    ...
    $.jBox.lxyAlert = function(content,title,submit){
    	 var defaults = {
	            buttons: eval('({ "' + $.jBox.languageDefaults.ok + '": "ok"})')
    	  };
    ...
    $.jBox.confirm1 = function (content, title, submit, options) {
        var defaults = {
            buttons: eval('({ "' + $.jBox.languageDefaults.ok + '": "ok", "' + $.jBox.languageDefaults.cancel + '": "cancel" })')
        };
        if (submit != undefined && typeof submit == 'function') {
        ...
        options = $.extend({}, defaults, options);
        $.jBox.prompt(content, title, '', options);
    };
