	...
	<div class="panel panel-default mt-40">
		...
		<div class="panel-body">
						  	...
						  	<table>
								<tbody>
									<tr>
										...
										<td>
											<input id="ok" class="btn btn-green size-S radius ml-20" type="button" value="确定">
										</td>
										<td>
											<input id="clean" class="btn btn-green size-S radius ml-20" type="button" value="清空">
										</td>
									</tr>
								</tbody>
							</table>
							...
							<tbody class="text-c">
								<tr>
									...
									<td width="20%" id="">
										...
										<div class="div-btn" id="cancle-order" onclick="cancelOrder(this);" style="display:none;">
											取消订单
										</div>
			...
			<div id="tab-content" class="mt-40" style="min-height:500px;">
				<div id="c0" class="panel tab-content" style="display:none;">
				...
				<div id="c1" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
				...
				<div id="c2" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
				...
				<div id="c3" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
				...
				<div id="c4" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
				...
				<div id="c5" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
				...
				<div id="c6" class="panel tab-content" style="display:none;">
					<div class="panel-header">
						<ul class="cl text-c">
							<li style="width:20%;"><span>上团日期</span></li>
							<li style="width:20%;"><span>行程简述</span></li>
...
function goPay(obj){
	...
	location.href = "lxy/order/${appUser.USERNAME}/payment/"+orderId;
}
...
function deleteOrder(obj){
	layer.confirm('您确定要删除该订单吗?', {
		...
		function(){
			var orderId = $(obj).parent().find("#orderId").val();
			$.ajax({
				url : "lxy/order/delOrder/"+orderId,
				post : "post",
...
function viewOrder(obj){
	var orderId = $(obj).parent().find("#orderId").val();
	window.location.href="<%=basePath %>lxy/order/${appUser.USERNAME}/detail/"+orderId+".html";
}
...
function cancelOrder(obj){
	...
	layer.confirm('您确定要取消该订单吗?', {
		...
		function(){
			$.ajax({
				url : "lxy/order/cancelOrder/"+orderId,
				post : "post",
				success : function(data) {
					if (data.status == "success") {
						layer.msg("取消成功!");
...
function refundOrder(obj){
	...
	layer.open({
		  ...
		  yes: function(index, layero){
			  ...
			  $.post('<%=basePath %>lxy/order/saveRefund',param,function(res){
		...
		if(res.status == 'success'){
			...
			if(Number(res.list.length) != 0){
				for(var i=0;i<res.list.length;i++){
					...
					item.find("#orderCode").text(res.list[i].orderCode);
					...
					if(res.list[i].price != undefined){
						item.find("#price").text("￥"+parseFloat(res.list[i].price).toFixed(2));
					}else{
...
function setOrderState(obj, state){
	if(state == "WAITPAY"){
		...
		obj.find("#go-pay").show();
	}else if(state=="<%=XConst.ORDER_STATE.CANCELED%>"){
		obj.find("#status").text("已取消");
		obj.find("#delete-order").show();
	}
}
...
function getOrderState(state){
	if(state == "审核中"){
		...
		orderState="<%=XConst.ORDER_STATE.CANCELED%>";
	}else{
		orderState="";
	}
