...
<section class="container submit-order">
					...
					<nav class="nav navbar-nav nav-collapse" role="navigation" id="Hui-navbar">
						<ul class="cl">
							<c:if test="${orderForm.orderState =='WAITPAY' }">
								<li><a href="lxy/order/${appUser.USERNAME}/payment/${orderForm.id}.html">去支付</a></li>
							</c:if>
							<li><a href="javascript:printOrder();">打印订单</a></li>
							<li><a href="lxy/order/${appUser.USERNAME}/editUI/${orderForm.id}.html">更改订单</a></li>
							<!-- <li><a href="">取消订单</a></li> -->
						</ul>
					</nav>
				</div>
				...
				<strong>
					...
					<c:if test="${orderForm.orderState eq 'CHECKFAIL'}">
						...
						<div class="reason" style="font-size:15px;color:#877F7F;">
							<span>原因：</span><span>${orderForm.failReason }</span>
						</div>
					</c:if>
					<c:if test="${orderForm.orderState =='WAITPAY' }">
						订单已生成，等待支付
					...
					<c:if test="${orderForm.orderState =='REFUNDING_FAIL' }">
						退款失败
						<div class="reason" style="font-size:15px;color:#877F7F;">
							<span>原因：</span><span>${orderForm.reason }</span>
						</div>
					</c:if>
		...
		<div class="panel-body mt-20">
			<div class="panel panel-default">
				<div class="panel-body bd-bottom">
					<div class="cl">
						<span class="left f-l">上&nbsp;团&nbsp;日&nbsp;期&nbsp;:</span>
						<span class="f-l ml-20"><fmt:formatDate value="${orderForm.startTime}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></span>
							<c:if test="${orderForm.jtSite != null && orderForm.jtSite != '' }">
								<span class="f-l ml-40">${orderForm.jtSite} <c:if
										test="${orderForm.jtSiteKm != null }">(${orderForm.jtSiteKm}km)</c:if>
									接
								</span>
							</c:if>
						</div>
				...
				<div class="panel-body bd-bottom">
					<div class="cl">
						...
						<span class="f-l ml-20"><fmt:formatDate value="${orderForm.endTime }" pattern="yyyy-MM-dd"/></span>
							<c:if test="${orderForm.stSite != null && orderForm.stSite != '' }">
								<span class="f-l ml-40">${orderForm.stSite} <c:if
										test="${orderForm.stSiteKm != null }">(${orderForm.stSiteKm}km)</c:if>
									送
								</span>
							</c:if>
						</div>
						...
						<div class="ml-20 f-l">
							<c:forEach items="${cars }" var="car">
								<ul class="cl">
									...
									<c:if test="${car.driverName != '' && car.driverName != null }">
										<li class="f-l ml-50">司机（${car.driverName}|${car.driverTel }）</li>
									</c:if>
								</ul>
							</c:forEach>
						</div>
				...
				<div class="panel-body bd-bottom">
					<div class="cl">
						...
						<span class="f-l ml-20">${orderForm.route }
				...
				<div class="panel-body bd-bottom">
					<div class="cl">
						...
						<c:if test="${orderForm.guide != null && orderForm.guide != '' }">
							<span class="f-l ml-20">${orderForm.guide }（${orderForm.guideContactNumber }）</span>
						</c:if>
						<c:if test="${orderForm.guide == null ||  orderForm.guide == '' }">
							<span class="f-l ml-20">--</span>
						</c:if>
					</div>
				</div>
				<div class="panel-body">
					<div class="cl">
						...
						<c:if test="${orderForm.remark != ''}">
							<span class="f-l ml-20">${orderForm.remark }</span>
						</c:if>
						<c:if test="${orderForm.remark == ''}">
							<span class="f-l ml-20">--</span>
						</c:if>
			...
			<div class="cl f-18">
				<div class="f-l ml-30">
					<ul>
						...
						<li class="pd-5">下单时间&nbsp:<span class="ml-20"><fmt:formatDate value="${orderForm.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></span></li>
					</ul>
				</div>
				<div class="f-r mr-50">
					<c:if test="${orderForm.price!=null && orderForm.price!='' }">
						<ul>
							<li class="mt-40">总公里数:<span class="ml-20">${orderForm.km }km</span></li>
							<li class="mt-10">订单总计:<span class="ml-20">￥<fmt:formatNumber  type="number"  pattern="#.00" value="${orderForm.price }" /></span></li>
						</ul>
					</c:if>
				</div>
			</div>
	...
	<div id="print_style" class="" style=" display:none">
		<section class="pd-20">
			<div class="text-c f-24"><span><strong>${orderForm.roleName}</strong></span></div>
			<div class="panel panel-default">
				<div class="panel-body bd-bottom">
					<div class="cl">
				...
				<div class="panel-body bd-bottom">
					<div class="cl">
						<div class="row cl f-l col-xs-6">
							<span>上团日期：<span class="ml-10"><fmt:formatDate value="${orderForm.startTime }" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></span></span>
						</div>
						<div class="row cl f-r col-xs-6">
							<span>下团日期：<span class="ml-10"><fmt:formatDate value="${orderForm.endTime }" pattern="yyyy-MM-dd"/></span></span>
						</div>
					</div>
				</div>
				<div class="panel-body bd-bottom">
					<div class="cl">
						<div class="row cl f-l col-xs-6">
							<span>接团地点：
							<c:if test="${orderForm.jtSite != null && orderForm.jtSite != '' }">
								<span class="ml-10">${orderForm.jtSite } 
									<c:if test="${orderForm.jtSiteKm != null }">(${orderForm.jtSiteKm}km)</c:if>
						...
						<div class="row cl f-r col-xs-6">
							<span>送团地点：
								<c:if test="${orderForm.stSite != null && orderForm.stSite != '' }">
										<span class="ml-10">${orderForm.stSite } 
											<c:if test="${orderForm.stSiteKm != null && orderForm.stSiteKm != ''}">(${orderForm.stSiteKm}km)</c:if>
										</span>
								</c:if> 
								</span>
						</div>
				...
				<div class="panel-body bd-bottom">
					<div class="cl">
						<div class="row cl f-l col-xs-10">
						<span>导游：
								<c:if test="${orderForm.guide != '' }">
									<span class="ml-10">${orderForm.guide }（${orderForm.guideContactNumber }）</span>
								</c:if>
								<c:if test="${orderForm.guide == '' }">
					...
					<div class="cl">
						<div class="row cl f-l col-xs-6">
							<ul>
								...
								<li>下单日期&nbsp:<span class="ml-20"><fmt:formatDate value="${orderForm.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></span></li>
							</ul>
						</div>
						<div class="row cl f-r col-xs-6">
							<c:if test="${orderForm.price!=null && orderForm.price!='' }">
								<ul>
									...
									<li>订单总计:<span class="ml-20">￥<fmt:formatNumber  type="number"  pattern="#.00" value="${orderForm.price }" /></span></li>
								</ul>
							</c:if>
						</div>
					</div>
