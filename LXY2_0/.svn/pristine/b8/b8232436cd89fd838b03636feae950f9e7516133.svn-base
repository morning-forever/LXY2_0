		...
		<div class="panel-body">
			<div class="cl">
				...
				<c:choose >
					...
					<c:when test="${formData.isNeedCheck && formData.isPrice}">
						<span class="f-r mr-40 f-16">总价:<span class="f-26">￥<fmt:formatNumber  type="number"  pattern="#.00" value="${formData.price}" />(所填备注需要审核)</span></span>
					</c:when>
					<c:when test="${!formData.isNeedCheck}">
						<span class="f-r mr-40 f-16">总价:<span class="f-26">￥<fmt:formatNumber  type="number"  pattern="#.00" value="${formData.price}" /></span></span>
					</c:when>
				</c:choose>
			</div>
		</div>
	...
	$.post("lxy/order/submit",function(res){
		if(res.status == 'success'){
			...
			if(res.isNeedCheck){
				...
				location.href = "lxy/order/${appUser.USERNAME}/payment/"+res.orderId;
			}
			
		}else if(res.status == 'already_submit'){
