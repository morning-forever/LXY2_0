        ...
        "columnDefs": [
			...
			},
			{
 	            "targets": 13,
 	           	 render: function (a, b, c, d) {
	            	var context ;
	            	if(c.orderState == 'WAITPAY' || c.orderState == 'DOWNPAYMENT'){
	            		context = 
	 	                {
	 	                    func: [
	            	...
	            	} else if(c.orderState == '<%=XConst.ORDER_STATE.CHECKING%>'){
	            		context = 
	 	                {
	            				func: [
		   	 	                    {"name": "审核", "fn": "doCheck("+c.id+")", "type": "info", "icon":"pencil"},
		   	 	                    {"name": "查看", "fn": "toDetail(\'" + c.id + "\',\'" + c.nickname + "\')", "type": "light", "icon":"eye-open"},
		   	 	                    {"name": "删除", "fn": "doDelete(\'" + c.id + "\')", "type": "danger", "icon":"trash"}
	   	 	                    ]
	 	                };
	            	}else if(c.orderState == '<%=XConst.ORDER_STATE.PAY_OK%>'){
	            		if(c.isSmsDri == 0){
	            			context = 
		            		{
		 	                    func: [
